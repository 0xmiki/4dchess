...
it('pairs compatible guests atomically, assigns opposite seats, and retries the same search', async () => {
	...
	const first = await a.mutation(api.matchmaking.join, args);
	expect(first.status).toBe('waiting');
	expect(await a.mutation(api.matchmaking.join, args)).toEqual(first);
	await other.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '3+2' });
	...
	expect(
		(await a.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '3+2' })).gameId
	).toBe(paired.gameId);
	await expect(other.mutation(api.matchmaking.cancel, { id: first.id })).rejects.toThrow(
		'SEARCH_NOT_FOUND'
	);
	expect((await a.mutation(api.matchmaking.join, args)).gameId).toBe(paired.gameId);
});
it('does not pair stale leases or resurrect a cancelled request', async () => {
	...
	const search = await a.mutation(api.matchmaking.join, args);
	vi.setSystemTime(search.expiresAt + 1);
	expect((await a.mutation(api.matchmaking.heartbeat, { id: search.id })).status).toBe('cancelled');
	expect(
		(await b.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '10+5' }))
			.status
	).toBe('waiting');
	expect((await a.mutation(api.matchmaking.join, args)).status).toBe('cancelled');
});
it('serializes cancellation against pairing without abandoning a created game', async () => {
	...
	const [cancelled, joined] = await Promise.all([
		...
		b.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '3+2' })
	]);
...
it('lets only one competing guest claim a waiting player', async () => {
	...
	const results = await Promise.all(
		[b, c].map((player) =>
			player.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '3+2' })
		)
	);
...
it('prevents a friend invitation from starting a second concurrent game', async () => {
	...
	await a.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '3+2' });
	await b.mutation(api.matchmaking.join, { requestId: randomUUID(), timeControl: '3+2' });
...
it('remembers cancellation before a delayed join reaches the server', async () => {
	...
	expect((await player.mutation(api.matchmaking.join, request)).status).toBe('cancelled');
