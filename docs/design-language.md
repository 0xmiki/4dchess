...
4D chess opens with a choice to play. The board is the main visual: four flat boards beside their tesseract projection. Teaching lives on `/how-to-play`, separate from the returning player's path.
...
- Home: two raised play cards, optional return-to-match link, starting position, and a link to learn.
...
| Role                | Treatment                                                       |
| ------------------- | --------------------------------------------------------------- |
| Background          | Warm charcoal with plum surfaces.                               |
| Primary play action | Gold fill, dark text, darker gold lower edge.                   |
| Secondary controls  | Raised plum, ivory text.                                        |
| Fourth dimension    | Violet dashed connections, labelled W.                          |
| Board               | Warm ivory and muted violet squares.                            |
| Last move           | Gold-tinted squares.                                            |
| Selection           | Violet square with an inset boundary.                           |
...
| Pieces              | Ivory and dark plum. Floating dark pieces have a light outline. |
...
Buttons have a four-pixel lower edge. Pressing moves the face down by the same amount and collapses the shadow; hover brightens the face. `--motion-press` controls the transition. Reduced motion removes the transition.

Pointer interaction never draws a ring around the tesseract. Keyboard focus changes its background instead. Native buttons and board cells retain keyboard focus indicators, and diagrams retain arrow-key rotation and Home reset.
