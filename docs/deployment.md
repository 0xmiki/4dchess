...
| `CONVEX_DEPLOY_KEY`     | Deploy key for the backend this site should use                            |
| `AUTH_PROXY_SECRET`     | The same value already configured on that backend                          |

Set the repository variable `CLOUDFLARE_DEPLOY_ENABLED` to `true`. Until then, only checks run. You may add environment approval rules in GitHub. Pull requests never receive deployment secrets or publish a Worker.
